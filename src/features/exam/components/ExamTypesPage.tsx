...
function ExamTypeForm({
  ...
  return (
    <form
      ...
      className="space-y-4"
    >
      <Field label="Exam type name" error={errors.name?.message} hint="e.g. Term Test, Mock, Final Exam">
        <Input
          type="text"
      ...
      <div className="flex items-center justify-end gap-2 border-t border-neutral-100 pt-4">
        ...
        <Button text={isSubmitting ? "Saving…" : submitLabel} loading={isSubmitting} disabled={isSubmitting} className="w-auto" />
      </div>
    </form>
  );
}
  ...
  },
  {
    ...
    render: (type) => (
      <RowActions
        actions={[
          ...(canUpdate
            ? [{ label: "Edit", icon: <PencilIcon className="size-3.5" />, onClick: () => onEdit(type) }]
            : []),
          ...(canDelete
            ? [{ label: "Remove", icon: <TrashIcon className="size-3.5" />, danger: true, onClick: () => onDelete(type) }]
            : []),
        ]}
      />
    ),
...
export function ExamTypesPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(err, "Could not remove the exam type. It may still be in use."));
    } finally {
      setDeleteBusy(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        actions={
          canCreate ? (
            <Button text="New Exam Type" icon={<PlusIcon className="size-4" />} className="w-auto" onClick={() => setCreateOpen(true)} />
          ) : undefined
        }
      />
      ...
      {canCreate && (
        <Dialog open={createOpen} onClose={() => setCreateOpen(false)} title="New Exam Type" description="Add a category for grouping exams.">
          <ExamTypeForm submitLabel="Add Exam Type" onSubmit={handleAdd} onClose={() => setCreateOpen(false)} />
        </Dialog>
      )}

      {canUpdate && (
        <Dialog open={editTarget !== null} onClose={() => setEditTarget(null)} title="Edit Exam Type" description="Update the exam type name.">
          {editTarget && (
            <ExamTypeForm initial={editTarget.name} submitLabel="Save Changes" onSubmit={handleUpdate} onClose={() => setEditTarget(null)} />
          )}
        </Dialog>
      )}
