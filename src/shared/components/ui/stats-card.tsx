...
export function StatsCard({
  ...
  const body = (
    <div className="group flex min-h-full items-start justify-between gap-3 rounded-[18px] border border-neutral-200 bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,.02)] transition hover:-translate-y-0.5 hover:border-neutral-300 hover:shadow-sm sm:p-5">
      <div className="min-w-0">
        <p className="type-kpi-label">{label}</p>
        <p className="type-kpi mt-2 break-words">{value}</p>
        {delta && (
          <p
            className={cn(
